...
filepath = 'twstockyear2019_test5.csv'
...
if not os.path.isfile(filepath):  #如果檔案不存在就建立檔案
    ...
    for i in range(1,7):  #設定下載的月份
        stock = twstock.Stock('9939')  # 建立 Stock 物件
        stocklist=stock.fetch(2023,i)  # stocklist 是存放第 "i" 月的 "所有交易日" 的 "所有" 股價資料
...
plot({"data": data, "layout": Layout(title='2023年個股統計圖')},auto_open=True)
